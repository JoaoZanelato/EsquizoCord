...
router.get("/", async function (req, res, next) {
  try {
    ...
    console.error("Erro ao buscar a lista de utilizadores:", err);
    next(err);
  }
});
...
router.get("/:id/profile", requireLogin, async (req, res, next) => {
  ...
  try {
    const [users] = await pool.query(
      "SELECT id_usuario, Nome, FotoPerfil, Biografia, data_cadastro FROM Usuarios WHERE id_usuario = ?",
      [id]
    );
    if (users.length === 0) {
      return res.status(404).json({ message: "Utilizador não encontrado." });
...
// ROTA ATUALIZADA PARA INCLUIR data_cadastro
