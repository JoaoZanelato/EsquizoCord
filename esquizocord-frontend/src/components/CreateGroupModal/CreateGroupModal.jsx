...
import {
    Form, FormGroup, Label, Input, CheckboxContainer,
    ModalActions, CancelButton, SubmitButton
} from './styles';
import ImageCropModal from '../ImageCropModal/ImageCropModal';
import { HiddenFileInput, CustomFileUploadButton, PreviewImage } from '../ImageCropModal/styles';
...
const CreateGroupModal = ({ isOpen, onClose, onGroupCreated }) => {
    ...
    const handleFileSelect = (e) => {
        if (e.target.files && e.target.files.length > 0) {
            ...
            reader.addEventListener('load', () => setFotoOriginal(reader.result.toString() || ''));
        ...
        <>
            <ModalOverlay $isOpen={isOpen} onClick={handleClose}>
                <ModalContent onClick={(e) => e.stopPropagation()}>
                    ...
                    <Title as="h3" style={{ textAlign: 'center', marginBottom: '20px' }}>Crie o seu servidor</Title>

                    <Form onSubmit={handleSubmit}>
                        <FormGroup>
                            <Label htmlFor="group-name">Nome do Servidor</Label>
                            <Input id="group-name" type="text" value={nome} onChange={(e) => setNome(e.target.value)} required />
                        </FormGroup>
                        ...
                        <FormGroup>
                            ...
                            {fotoPreview && <PreviewImage src={fotoPreview} alt="Prévia da foto do grupo" />}
                        </FormGroup>

                        <CheckboxContainer>
                            <Input type="checkbox" id="group-private" checked={isPrivate} onChange={(e) => setIsPrivate(e.target.checked)} style={{width: 'auto'}} />
                            <label htmlFor="group-private">Grupo Privado</label>
                        </CheckboxContainer>

                        <ModalActions>
                            <CancelButton type="button" onClick={handleClose}>Cancelar</CancelButton>
                            <SubmitButton type="submit" disabled={isSubmitting || !nome.trim()}>
