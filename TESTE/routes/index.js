...
router.post('/verificar-email', async (req, res, next) => {
  ...
  try {
    const pool = req.db;const express = require('express');
const router = express.Router();
const bcrypt = require('bcrypt');
const multer = require('multer');
const saltRounds = 10;
...
router.get('/esqueceu-senha', (req, res) => {
    res.render('Esqueceu-senha', { message: null });
});
...
// Rota para processar o pedido de recuperação de senha
router.post('/esqueceu-senha', async (req, res, next) => {
    try {
        ...
        if (users.length === 0) {
            return res.render('Esqueceu-senha', { message: 'Se o e-mail estiver registado, um link de recuperação será enviado.' });
        }
        ...
        res.render('Esqueceu-senha', { message: 'Se o e-mail estiver registado, um link de recuperação será enviado.' });
...
router.post('/redefinir-senha', async (req, res, next) => {
    try {
        ...
        // Redireciona para a página de login, idealmente com uma mensagem de sucesso
        res.redirect('/login'); 
